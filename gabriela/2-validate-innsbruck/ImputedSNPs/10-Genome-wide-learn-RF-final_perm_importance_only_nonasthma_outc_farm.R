...
train.fin.ind <- ind.noinn[gabriel.1707$dd_asthma[ind.noinn] == 0 ]
test.fin.ind <- ind.part$all.inn[gabriel.1707$dd_asthma[ind.part$all.inn]==0 ]
...
train.ranger <- ranger::holdoutRF(dependent.variable.name = "y", data = dat, write.forest = T, 
                       save.memory = TRUE, probability = TRUE, num.trees =ntrees, num.threads = 15, verbose = FALSE
) 
...
save(train.ranger, importance_janitza, file=paste0(
  "2-validate-innsbruck/ImputedSNPs/Results_genomewide/ranger_object_genomewide_final_nonasthma_outcomefarm.rda"))
...
save(train.ranger2, y.inn, preds , file=paste0(
   "2-validate-innsbruck/ImputedSNPs/Results_genomewide/ranger_prediction_object_genomewide_final_nonasthma_outcomefarm.rda"))
