...
do.for.imp.set <- function(i){
  ...
  x.interact <- model.matrix(  form.interact,
                               data= data.frame(snp.data[ ,-1], x.env[ , c("Num_Sibs_12", "farm2", fam.an )], x.conf[ , "Sex_female2", drop=FALSE] ))
  ...
  # temporary:
  # env.ifs <-
  # env.ifs <- c( "conf_only", "snps_only", "env_only", "fh_only", "interactions_only" ,  "both", "conf_only")
