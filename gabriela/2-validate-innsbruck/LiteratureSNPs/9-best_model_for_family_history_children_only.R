...
### fh==0: fhasthma2==1 to be replaced by fhasthma2==0
# save(aucs.imps, file="Results19/aucs/aucs_fh0_ranger_fh_conf_snps.rda") 



### Variable importance for one imputation since mostly SNPs:
# we take imputations 5
set.seed(2323)
train.ranger.fin  <- ranger(dependent.variable.name = "y", data = dat.fh1, write.forest = T, 
                       save.memory = FALSE, probability = TRUE, num.trees =ntrees, num.threads = 8, verbose = FALSE, importance="permutation")
importance_altmann <- importance_pvalues(train.ranger.fin, method="altmann", formula = formula(y~.), data = dat.fh1)

save(aucs.imps, file = "Results19/finalmodel/variable_importance_ranger_fh1_conf_snps.rda")


# 
# vars.imp.altmann <-  importance_altmann[ ,"importance" ]
# 
# ### the following code in comments gives the same as above...
# means.vars.imp.altmann <-vars.imp.altmann
...
# 
# info.vars.sort.altman <- convert.variable.names(names.vars.sort.altmann)
# 
# pvals.imp.altmann <- importance_altmann[ ,"pvalue" ]
# pvals.imp.altmann[pvals.imp.altmann < 0.05]
