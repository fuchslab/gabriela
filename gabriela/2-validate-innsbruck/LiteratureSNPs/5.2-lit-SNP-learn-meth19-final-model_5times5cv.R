...
do.for.imp.set <- function(i, env.if="both", method2=NULL){
  ...
  # NEWMARK
  ##################################### formula farm interactions 2
  # since fh_conf_snps best for onlyfarm (and after decrease)
  # + since fh_conf_env best for onlynonfarm (and after decrease): (version 2)
  # use follwing covariates: (fh + conf)*farm + env:abs(1-farm)??? + snps:farm
  
  form.farm.interact2.char <- paste( "~(", paste(colnames( snp.data[,-1]), collapse="+"), ")", ":", 
                                     "farm2 + (", paste(colnames(x.env.nofh)[- which(colnames(x.env.nofh)=="farm2")], collapse="+"), ")", ":", "I(1-farm2)"
  )
  
  form.farm.interact2 <- formula(form.farm.interact2.char)
  
  # NEWMARK
  x.farm.interact2 <- model.matrix(  form.farm.interact2,
                                     data= data.frame(cbind( x.fh,x.conf, x.env.nofh,  snp.data[,-1])))
  
  
  x.env.3blocks.farm.interact2 <- cbind(x.env.nofh, x.fh, x.farm.interact2)
  
  blocks.farm.interact2.ipf <- list(1:ncol(x.conf), # confounders
                                    (ncol(x.conf)+1):(ncol(x.conf) + ncol(x.env.nofh)), # environmnet
                                    (ncol(x.conf) + ncol(x.env.nofh) + 1): (ncol(x.conf) + ncol(x.env.nofh) + ncol(x.fh)), # family history
                                    (ncol(x.conf) + ncol(x.env.nofh) + ncol(x.fh)+1): (ncol(x.conf) + ncol(x.env.nofh) + ncol(x.fh) + ncol(x.farm.interact2)), # interactions
                                    (ncol(x.conf) + ncol(x.env.nofh) + ncol(x.fh) + ncol(x.farm.interact2)+1):
                                      (ncol(x.conf) + ncol(x.env.nofh) + ncol(x.fh) + ncol(x.farm.interact2)+ncol(snp.data[,-1])) # snps
  )
  ...
  get.env <- function(env.label){
    ...
    if(env.label=="fh_conf_env_snps_farm.interact1") x.env <- x.env.3blocks.farm.interact2
    x.env 
  }
  ...
  model.19.final <- function( y.label, method, env.if=env.if, obs.weights=gabriel.1707$weight_dna){
    ...
    ######################################
    ######## modelling part (still ident. to 3-wrp...)
    if(method=="ranger"){
      require(ranger)
      ...
      train.glmnet <- ranger(dependent.variable.name = "y", data = data.frame(y=factor(y[train.fin.ind2]),x.pred[ train.fin.ind2, ]), write.forest = T, 
                             save.memory = TRUE, probability = TRUE, num.trees =ntrees, num.threads = 8, verbose = FALSE) 
      pred <- predict(train.glmnet, data= data.frame(x.pred[ test.fin.ind2, ]) ) $predictions[ ,2]
    }
    
    if(method=="elnet_weights"){
      require(glmnet)
      train.glmnet <- cv.glmnet(y=y[train.fin.ind2], x=x.pred[ train.fin.ind2, ], family="binomial", type.measure="auc",
                                alpha=0.5, weights=obs.weights[ train.fin.ind2])
      pred <- predict(train.glmnet, newx = x.pred[ test.fin.ind2, ], type="response")
    }
    ...
    if(method=="ipflasso" & env.if=="both"){
      ...
      train.glmnet <- cvr2.ipflasso(Y=y[train.fin.ind2], X=x.pred[ train.fin.ind2, ], family="binomial", type.measure="auc", alpha=1, 
                                    blocks=blocks, pflist=pflist,
                                    nfolds=5,ncv=5)
      pred <- ipflasso.predict(train.glmnet, Xtest = x.pred[ test.fin.ind2, ])$probabilitiestest
    } 
    ...
    # plot(perf)
    list(model.obj=train.glmnet, pred = pred, 
         perf.values = list(roc.a=roc.a, roc.b.i = roc.b.i, roc.b.ii, mult.preds=mult.preds ))
  }
  
  
  if(env.if=="both" & is.null(method2)){
    do.for.all.outcomes <- function(y.label){
      obj.fin.both <- model.19.final(env.if="both", y.label=y.label, method = "ipflasso_weights")
      save(obj.fin.both, file=paste0("Results19/finalmodel/fin.models.", y.label, ".imp",i,"_weightscv5times.rda" ))
    }
    mclapply(c( "dda"), do.for.all.outcomes, mc.cores = 1)
  }
  # mclapply(c("dda_notany", "dda.wh12_notany"), do.for.all.outcomes, mc.cores = 2)
  if(env.if=="fh_only"& is.null(method2)){
    
    # same for other modalities
    do.for.all.outcomes <- function(y.label){
      obj.fin.both <- model.19.final(env.if="fh_only", y.label=y.label, method = "ranger")
      save(obj.fin.both, file=paste0("Results19/finalmodel/fin.models.", y.label, ".imp",i,"_fh_only_rangercv5times.rda" ))
    }
    mclapply(c("dda"), do.for.all.outcomes, mc.cores = 1)
    # mclapply(c("dda_notany", "dda.wh12_notany"), do.for.all.outcomes, mc.cores = 2)
  }
  else if(method2 == "elnet_weights"){
    y.label="dda"
    obj.fin.both <- model.19.final(env.if=env.if, y.label=y.label, method = method2)
    save(obj.fin.both, file=paste0("Results19/finalmodel/fin.models.", y.label, ".imp",i,"_", env.if ,"_", method2,"cv5times.rda" ))
  }
