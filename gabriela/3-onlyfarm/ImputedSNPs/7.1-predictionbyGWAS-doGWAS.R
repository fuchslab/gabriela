...
for(i in 1:22){
    ...
    for(j in 1:5){
      # NEWMARK
      train.train.index <- setdiff( 1:length(unlist(ind.part$cvfolds.without.inn$farm)), ind.part$cvfolds.without.inn$farm[[j]])
...
## change path!
setwd("~/../../../storage/cmbstore/norbert.krautenbacher/1-Projects/2-Asthma")
save(l.snp.pvals, file = "3-onlyfarm/ImputedSNPs/Data/l.snp.pvals.rda")  
