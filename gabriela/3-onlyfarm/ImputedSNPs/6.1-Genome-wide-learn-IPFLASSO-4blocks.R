...
do.for.imp.set <- function(i,  env.ifs, snp.as){
  ...
  get.blocks.ipf <- function(env.label){
    if(env.label == "both"){
      ...
      blocks.ipf <- list(1:ncol(x.fh),
                         (ncol(x.fh)+1):(ncol(x.fh) + ncol(x.conf)),
                         (ncol(x.fh) + ncol(x.conf) +1): (ncol(x.fh) + ncol(x.conf) + ncol(snp.data) )
      )
    }else if(env.label == "fh_conf_env_snps"){
      blocks.ipf <- list(1:ncol(x.fh),
                         (ncol(x.fh)+1):(ncol(x.fh) + ncol(x.conf)),
                         (ncol(x.fh) + ncol(x.conf) +1): (ncol(x.fh) + ncol(x.conf) + ncol(x.env.nofh)), 
                         (ncol(x.fh) + ncol(x.conf) + ncol(x.env.nofh) +1): (ncol(x.fh) + ncol(x.conf) + ncol(x.env.nofh) + ncol(snp.data))                
      )
    }
    blocks.ipf
  }
  ...
  for(method in methods){
    for(env.if in env.ifs){
      ...
      perform.asthma(y=get.y(y.label), snp.as=snp.as, env.if=env.if, 
                     method=method, x.env=get.env(env.if), snp.data=snp.data, 
                     obs.weights=obs.weights, x.conf=x.conf, blocks.ipf=get.blocks.ipf(env.if), n.cores=2,
                     #### MARKNEW
                     ## change path!
                     path.results=paste0("3-onlyfarm/ImputedSNPs/Results_genomewide/imp",i,"/"), 
                     needed.as.mat=TRUE,needed.as.df=FALSE, ntrees=20000, conv.x.all.to.model.matrix=FALSE )
    } 
...
# mclapply(1:5, function(x) do.for.imp.set(x,  env.ifs = "both", snp.as = "genomewide"), mc.cores=5) # careful: inner core specification
mclapply(1:5, function(x) do.for.imp.set(x,  env.ifs = "fh_conf_snps", snp.as = "genomewide"), mc.cores=5) # careful: inner core specification
