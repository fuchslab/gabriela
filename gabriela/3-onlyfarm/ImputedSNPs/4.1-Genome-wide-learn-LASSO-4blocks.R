  ...
  for(method in methods){
    for(env.if in env.ifs){
      ...
      perform.asthma(y=get.y(y.label), snp.as=snp.as, env.if=env.if, 
                     method=method, x.env=get.env(env.if), snp.data=snp.data, 
                     obs.weights=obs.weights, x.conf=x.conf, blocks.ipf=blocks.ipf, n.cores=2,
                     #### MARKNEW
                     ## change path!
...
mclapply(1:5, function(x) do.for.imp.set(x,  env.ifs = "fh_conf_snps", snp.as = "genomewide"), mc.cores=5)
