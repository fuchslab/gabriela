...
save(train.ranger, importance_janitza, file=paste0("3-onlyfarm/ImputedSNPs/Results_genomewide/finalmodel/ranger_object_",env.if,"_imp",i,"_final.rda"))
...
train.ranger <- ranger::holdoutRF(dependent.variable.name = "y", data = dat.snp, write.forest = T, 
                                  save.memory = TRUE, probability = TRUE, num.trees =20000, num.threads = NULL, verbose = FALSE
) 
importance_janitza <- ranger::importance_pvalues(train.ranger, method = "janitza", dependent.variable.name = "y", data = dat) 
