...
do.for.imp.set <- function(i,  env.if){
...
if(env.if=="both"){
  ...
  # form <- formula(y ~.)
  dat<-data.frame(y=factor(c(y)$dda), x.all)
  ...
  # for big data:
  # train.ranger <- ranger(dependent.variable.name = "y", data = dat, write.forest = T, 
                         # save.memory = TRUE, probability = TRUE, num.trees =20000, num.threads = 10, verbose = FALSE
                          # , importance = "impurity"
                         # ) 
  
  xgb.train(...)
  
  save(train.ranger, file=paste0("3-onlyfarm/ImputedSNPs/Results_genomewide/finalmodel/ranger_object_",env.if,"_imp",i,".rda"))
...
mclapply(1:5, function(x) do.for.imp.set(x,  env.if = "snps_only"), mc.cores=5)
  ### Perform:
