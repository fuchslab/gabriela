...
###  visualize
B=10000
load(paste0("~/../../../storage/cmbstore/norbert.krautenbacher/1-Projects/2-Asthma/4-onlynonfarm/LiteratureSNPs/2-LearningMethods/Results19/aucs/all.aucs_for_synergy_models_with.tests.",B ,"bootstrap_samples.rda"))
env.ifs <- c("fh_only", "fh_conf", "fh_conf_env", "fh_conf_snps", "fh_conf_env_snps",  "both")
## check it out
all.aucs.syn$dda[ , c("method", env.ifs)]

barplot(as.matrix(all.aucs.syn$dda[ , env.ifs]), beside = T, ylim=c(0.5, 0.75), xpd=FALSE, main = "only nonfarm", legend.text = all.aucs.syn$dda$method)
