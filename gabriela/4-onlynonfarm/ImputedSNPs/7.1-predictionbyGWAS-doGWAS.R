...
for(i in 1:22){
    ...
    for(j in 1:5){
      # NEWMARK: inner folds for nonfarm!!
      train.train.index <- setdiff( 1:length(unlist(ind.part$cvfolds.without.inn$nonfarm)), ind.part$cvfolds.without.inn$nonfarm[[j]])
    ...
    save(l.snp.pvals, file=paste0("~/../../../storage/cmbstore/norbert.krautenbacher/1-Projects/2-Asthma/4-onlynonfarm/ImputedSNPs/Data/l.snp.pvals_till_chr",i , ".rda"))
...
## change path!
setwd("~/../../../storage/cmbstore/norbert.krautenbacher/1-Projects/2-Asthma")
save(l.snp.pvals, file = "4-onlynonfarm/ImputedSNPs/Data/l.snp.pvals.rda")  
