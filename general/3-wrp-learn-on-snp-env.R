...
#   
  ...
  if(env.if == "env_only" | env.if == "fh_only" |env.if ==  "interactions_only"){
    # MARKNEW (no potential confounders in single blocks)
  ...
  ############ new ones for synergy effect models
  ##### Note: variables contained in "x.env" (conf, fh, env)  is specified outside of this script
   if(env.if== "fh_conf"){
    x.all = x.env[ind.y, ]
    pflist = list(c(1,1),  # first block: confounders and  weight always set to 1 
                  c(1,2), 
                  c(1,3)) # weight two blocks three times
    rm(x.env)
  } else if(env.if=="fh_conf_env"){
    x.all = x.env[ind.y, ]
    ...
    rm(x.env)
  } else if(env.if=="fh_conf_snps"){
    x.all = cbind(x.env, x.snp)[ind.y, ]
    pflist = list(c(1,1,1),  # first block: confounders and  weight always set to 1 
                  c(2,1,1), c(1,1,2), # weight one block more
                  c(2,1,2), # weight two blocks more
                  c(3,1,1), c(1,1,3), # weight one block three times
  ...
  } else if(env.if=="fh_conf_env_snps"){
    x.all = cbind(x.env, x.snp)[ind.y, ]
    pflist = list(c(1,1,1,1),
                  c(2,1,1,1), c(1,1,2,1), c(1,1,1,2),
                  c(2,1,2,1), c(2,1,1,2), c(1,1,2,2), 
                  c(3,1,1,1), c(1,1,3,1), c(1,1,1,3),
                  c(3,1,3,1), c(3,1,1,3), c(1,1,3,3)
                  )
    rm(x.env)
  } else if(env.if=="fh_conf_env_snps_int"){
    stop("Use env.if=='both' ")
  } 
...
# pred <- list() 
  # for(i in 1:k){
    ...
    if(method=="ipflasso" & env.if=="both"){
      require(ipflasso)
        if( !is.null(blocks.ipf) ){
         ...
         stop("the blocks argument has to be not null if ipf-lasso should be performed")
        }
    ...
    if(method=="ipflasso_weights" & env.if=="both"){
      ...
      if(method=="ipflasso_weights" & (env.if== "fh_conf" | env.if== "fh_conf_env"| env.if== "fh_conf_snps" | env.if== "fh_conf_env_snps") ){
        
        if(is.null(pflist)) stop("pflist.other must be not-null, when env.if='other' for ipflasso !")
        
        require(ipflasso)
        if(is.null(blocks.ipf)){ 
          stop("the blocks.ipf argument has to be not null if ipf-lasso should be performed")
        }
        blocks=blocks.ipf
        train.glmnet <- cvr2.ipflasso.ed(Y=y[ind.train[[i]]], X=x.pred[ ind.train[[i]], ], family="binomial", type.measure="auc", alpha=1, 
                                         blocks=blocks, pflist=pflist,
...
#       }
   ...
   if(method=="glm_gwas"){ # careful: Wu 2013 uses j for subjects and j for snp
      ...
      for(j in 1:top.snp.no){
        ...
        dat.train.for.snp.j <- data.frame(y, x.pred[, top.snp.names[j], drop=FALSE], stringsAsFactors = FALSE)[ ind.train[[i]], ] # without conf
        # dat.train.for.snp.j <- data.frame(y, x.pred[, c(top.snp.names[j], colnames(x.conf))], stringsAsFactors = FALSE)[ ind.train[[i]], ] # with conf
